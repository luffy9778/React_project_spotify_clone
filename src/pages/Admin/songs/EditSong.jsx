...
const EditSong = () => {
    ...
    const fetchArtist = async () => {
      try {
        ...
        const { songname, artistname, songbgcolour, songtags } =
          response.data.song;
        setFetchedData(response.data.song);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const updatedData = {};
    if (fetchedData.songname !== songName) {
      updatedData.songname = songName;
    }
    if (fetchedData.artistname?.artistname !== artistName) {
      updatedData.artistname = artistName;
    }
    if (fetchedData.songbgcolour !== bgColour) {
      updatedData.songbgcolour = bgColour;
    }
    if (fetchedData.songtags?.tagName !== tags) {
      updatedData.songtags = tags;
    }
    if (image) {
      ...
      <form className="max-w-3xl mx-auto pt-5" onSubmit={handleSubmit}>
        ...
        <div className="flex gap-4 mb-6">
          ...
          <div className="mb-4 w-1/3 flex items-end justify-center ">
            <button
              type="submit"
              className="w-full h-12  text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg px-5 py-2.5 text-center me-2 mb-2"
            >
              Edit Song
            </button>
          </div>
